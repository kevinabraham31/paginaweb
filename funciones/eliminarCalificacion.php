...
{
    $elimina = "DELETE FROM cursado_alumnos WHERE (ncontrol = $id);";
    
    if (mysqli_query($conexion, $elimina)) {
        echo '<script>alert("Datos eliminados con éxito"); window.location.href = "CalificaciónAlumnado.php";</script>';
        exit;
    } else {
